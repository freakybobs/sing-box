...
func (a *Adapter) Start() error {
	outbounds := a.outbound.Outbounds()
	groups := make([]adapter.OutboundGroup, 0, len(outbounds))
	for _, out := range outbounds {
		if group, isGroup := out.(adapter.OutboundGroup); isGroup && group.HasProvider(a.providerTag) {
			groups = append(groups, group)
		}
	}
	a.groups = groups
	a.history = service.FromContext[*urltest.HistoryStorage](a.ctx)
	if a.history == nil {
...
func (a *Adapter) UpdateGroups() {
	for _, group := range a.groups {
		err := group.UpdateOutbounds(a.providerTag)
		if err != nil {
			a.logger.Error("update ", group.Type(), " ", group.Tag(), ": ", err)
		}
	}
}
	...
	for _, detour := range a.outbounds {
		...
		b.Go(tag, func() (any, error) {
			...
			if err != nil {
				...
				a.history.StoreURLTestHistory(tag, &urltest.History{
					Time:  time.Now(),
					Delay: t,
				})
				resultAccess.Lock()
