...
func ParseSubscriptionLink(link string) (option.Outbound, error) {
	...
	if len(result) == 0 {
		...
		isBase64 := reg.MatchString(result[3])
		if isBase64 && len(result[3])%4 == 0 {
...
func parseHeaders(headersRaw string) badoption.HTTPHeader {
...
func v2rayTransportWs(host string, path string) option.V2RayWebsocketOptions {
	...
	if host != "" {
		WebsocketOptions.Headers = parseHeaders(fmt.Sprint("Host:", host))
	}
	if path != "" {
		...
		if result[2] != "" {
			...
			WebsocketOptions.MaxEarlyData = stringToUint32(result[2])
		}
	}
...
func parseVMessLinik(link string) (option.Outbound, error) {
			...
			case "tcp":
				if tType, exists := proxy["type"]; exists {
					...
					if headers, exists := proxy["headers"]; exists {
						Transport.HTTPOptions.Headers = parseHeaders(headers)
					}
				}
			case "grpc":
