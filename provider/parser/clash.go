...
func convertDialerOption(proxy map[string]any) option.DialerOptions {
	...
	if mark, exists := proxy["routing-mark"].(uint32); exists {
		if runtime.GOOS == "android" || runtime.GOOS == "linux" {
			options.RoutingMark = option.FwMark(mark)
		}
	}
	...
	for _, proxy := range clashConfig.Proxies {
		...
		case "ss":
			if plugin, exists := proxy["plugin"]; exists {
				switch plugin {
				case "shadow-tls":
					outbound, stlsPart, err = parseSTLSClash(proxy)
				case "obfs", "v2ray-plugin":
					outbound, err = parseSSClash(proxy)
				default:
					continue
				}
			} else {
		...
		case "http":
			outbound, err = parseHTTPClash(proxy)
		case "tuic":
			if _, exists := proxy["token"]; exists {
				continue
			}
			outbound, err = parseTUICClash(proxy)
		case "vmess":
			outbound, err = parseVMessClash(proxy)
		case "vless":
			if flow, exists := proxy["flow"].(string); exists && flow != "xtls-rprx-vision" && flow != "" {
				continue
			}
			outbound, err = parseVLESSClash(proxy)
		case "socks5":
			outbound, err = parseSOCKS5Clash(proxy)
		case "trojan":
			if _, exists := proxy["flow"].(string); exists {
				continue
			}
			outbound, err = parseTrojanClash(proxy)
		case "hysteria":
...
func parseSSClash(proxy map[string]any) (option.Outbound, error) {
	outbound := option.Outbound{
...
func parseSTLSClash(proxy map[string]any) (option.Outbound, option.Outbound, error) {
	...
	ssPart.Options = ssOptions
	stlsPart.Options = stOptions
	return ssPart, stlsPart, nil
}

func parseHTTPClash(proxy map[string]any) (option.Outbound, error) {
	outbound := option.Outbound{
		Type: C.TypeHTTP,
	}
	...
	if headers, exists := proxy["headers"].(map[string]any); exists {
		options.Headers = map[string]badoption.Listable[string]{}
		for key, value := range headers {
			valueArr := badoption.Listable[string]{}
...
func parseTUICClash(proxy map[string]any) (option.Outbound, error) {
	outbound := option.Outbound{
		Type: C.TypeTUIC,
	}
...
func parseVMessClash(proxy map[string]any) (option.Outbound, error) {
	outbound := option.Outbound{
		Type: C.TypeVMess,
	}
...
func parseVLESSClash(proxy map[string]any) (option.Outbound, error) {
	outbound := option.Outbound{
		Type: C.TypeVLESS,
	}
...
func parseSOCKS5Clash(proxy map[string]any) (option.Outbound, error) {
	outbound := option.Outbound{
		Type: C.TypeSOCKS,
	}
...
func parseTrojanClash(proxy map[string]any) (option.Outbound, error) {
	outbound := option.Outbound{
		Type: C.TypeTrojan,
	}
...
func parseHysteriaClash(proxy map[string]any) (option.Outbound, error) {
	outbound := option.Outbound{
		Type: C.TypeHysteria,
	}
...
func parseHysteria2Clash(proxy map[string]any) (option.Outbound, error) {
	outbound := option.Outbound{
